...
---
# Background
[README](https://github.com/ros-planning/navigation2/blob/main/nav2_bringup/README.md)
The `nav2_bringup` package is a flexible example that can be modified to any robot, making it a great place to start with navigation.

Usual robot stacks will have a `<robot_name>_nav` package with config/bringup files and this is that for the general case to base a speciffic robot system off of. 


Dynamically composed bringup can be used to compose all Nav2 nodes into a single process instead of launching nodes separately. 
- Dynamically composed bringup is used by default. 

## Multi-robot Simulation
### Cloned
Allows bring up multiple robots, cloning a single robot N times at different positions on the map. The parameters are loaded from `nav2_multirobot_params_all.yaml` file by default
- The format for each robot is: `robot_name={x: 0.0, y: 0.0, yaw: 0.0, roll: 0.0, pitch: 0.0, yaw: 0.0}`
...
```
# Example
- [Multi-tb3 simulation launch ](https://github.com/ros-planning/navigation2/blob/main/nav2_bringup/launch/cloned_multi_tb3_simulation_launch.py)
```python
