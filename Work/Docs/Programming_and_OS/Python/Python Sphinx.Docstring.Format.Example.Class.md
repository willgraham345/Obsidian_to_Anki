...
"""
```
- `:param:` and `:type:` directive options must be used for each parameter we wish to document. 
- `:raises:` used for any errors that are raised by the code. 
- `:return:` and `:rtype:` options are used to describe any values returned by our code. 
