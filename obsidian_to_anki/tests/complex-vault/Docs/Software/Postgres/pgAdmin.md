...
Use 
 - [c] `docker inspect <database_name> -f "{{json .NetworkSettings.Networks }}"` = Finds docker networking ip for a PostgreSQL container (may work for other stuff too) = #tools/docker/networking
