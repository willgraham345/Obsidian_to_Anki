...
date modified: Friday, January 9th 2026, 2:24:04 pm
...
## Concepts of Note
- Downstream users of your code may benefit from installing your code rather than leaving it as a standalone exe.
- The install process creates links, files, shortcuts, and potentially register entries that are helpful.
...
- Projects can install files other than those created with [[CMake add_executable]] and [[CMake add_library]].
	- The install process is different than the build/source libraries. It can go wherever the user wants it to go. Specified by [[CMAKE_INSTALL_PREFIX]].
...
󰙎 `install(TARGETS ...)` ;;; Binary files corresponding to targets

󰙎 `install(FILES...)` ;;; General-purpose file installation
- Typically used for headers, documentation, and data files required by software

󰙎 `install(PROGRAMS...)` ;;; Executables not built by project
- Shell scripts

󰙎 `install(DIRECTORY …)` ;;; Entire directory tree
- Great for icons/images

󰙎 `install(SCRIPT …)` ;;; CMake Script file
...
󰙎 `install(CODE …)` ;;; CMake code during execution
...
󰙎 `install(EXPORT …)` ;;; Code to Import Targets
