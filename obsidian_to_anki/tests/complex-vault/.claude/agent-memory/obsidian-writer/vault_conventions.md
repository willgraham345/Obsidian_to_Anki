...
- `## Properties` — one `󰙎 term ;;; description` line per type/constant (preferred over tables for type-reference sections in Linux notes)
...
C++ compiler/language notes in `Docs/Programming_and_OS/C/`:
- `[[Cpp.compiler.overview]]` — parent note listing compilers (GCC, Clang, MSVC, Intel)
- `[[Cpp.Functions.Inline]]` — inline function detail
- `[[C.Storage_Class_Specifiers]]` — storage class keywords
- C++ notes use spaces in filenames (e.g. `Cpp compiler attribute extensions.md`) per user correction
- H2 headings for compiler attribute/extension notes: `## Syntax Forms`, `## Common Attributes`, `## Usage`, `## Portability`
