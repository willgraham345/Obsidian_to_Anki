...
date modified: Thursday, July 17th 2025, 2:50:44 pm
interfaces:
  - "[[Rust std IntoIterator]]"
  - "[[Rust std Iterator]]"
library_of:
  - "[[Rust std]]"
---
...
## Concepts of Note
󰙎  Adapters ;;; Functions which take an [[Rust std Iterator|Iterator]] as an input, and return another [[Rust std Iterator|Iterator]]. Often called "iterator adapters", as they're a form of the [[DP Adapter]]. Common examples include `map`, `take` and `filter`
...
󰠗  What trait is at the center of the `std iter` crate? What interface does it introduce? ;; `Iterator`, which has `type Item` and `fn next(&mut self) -> Option<Self::Item>` = #lang/data/iterations  
<!--ID: 1758253288571-->
