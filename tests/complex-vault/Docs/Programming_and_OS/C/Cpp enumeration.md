...
󰠗  What underlying type are I scoped enumerations? ;; `int`
󰙎  Scoped enumerations ;;; Strongly typed enumerations (introduced in C++11). Introduced using `::` to access enumerations and and using an underlying type for the enum. 
## Usage
  `enum enumName {enum1, enum2}` ;;; Declare an unscoped enum `e` with two different variants of the same type (`enum1, enum2`) (implicitly converted to an int) = #lang/data/enumeration  
ID: 1751997629744
