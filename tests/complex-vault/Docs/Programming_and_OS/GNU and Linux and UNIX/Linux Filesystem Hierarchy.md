...
processes:
  ...
  - "[[Linux Mounting another Filesystem]]"
concept_of:
  - "[[Linux]]"
date created: Tuesday, August 20th 2024, 2:05:34 pm
date modified: Tuesday, March 3rd 2026, 3:39:23 pm
items:
  - "[[Linux archive types]]"
  - "[[Linux disk partitioning]]"
  - "[[Linux fstab]]"
...
#### /dev/ptpN
- The typical PTP hardware clock exposed. Relies on a driver to implement the `ptp_clock_info`. The driver will also adjust the oscillator frequency and `adjtime()` for step corrections. 
...
### /etc/fstab
- The filesystem table (commonly found in Unix/Unix-like computer systems). Processed by `systemd` via `systemd-fstab-generator` for automatic mounting during boot. 
...
### /etc/ptp4l.conf
- Typical slave config for embedded node.
...
- Files that define how [[Linux systemd|systemd]] will handle a unit. Default location for these files
	- Read more at [[Linux systemd unit#Types of Units/Best Practice]]
...
#### proc/pid/ns
- Each process has a `proc/`PID`/ns/` subdirectory containing one entry for each namespace that supports being manipulated by [[Linux setns]]. See [[Linux namespaces]] for more information here.

#### proc/sys/user
The files in the _/proc/sys/user_ directory (which is present since Linux 4.9) expose limits on the number of namespaces of various types that can be created. See [[Linux namespaces]] for more info.
