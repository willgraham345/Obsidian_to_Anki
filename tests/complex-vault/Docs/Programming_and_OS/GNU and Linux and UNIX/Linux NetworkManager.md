...
date modified: Wednesday, November 19th 2025, 1:51:37 pm
tags: [cs/linux/networking]
template:
template-version:
---
...
# Usage
  `nmcli device wifi list` ;;; List nearby Wi-Fi networks = #cs/linux/networking 
ID: 1751997628772



  `nmcli device wifi connect _SSID_or_BSSID_ password _password_` ;;; Connect to a Wi-Fi network = #cs/linux/networking 
ID: 1751997628776



  `nmcli device wifi connect _SSID_or_BSSID_ password _password_ hidden yes` ;;; Connect to a hidden Wi-Fi network = #cs/linux/networking 
ID: 1751997628780



  `nmcli device wifi connect _SSID_or_BSSID_ password _password_ ifname wlan1 _profile_name_` ;;; Connect to a Wi-Fi on the `wlan1` interface = #cs/linux/networking 
ID: 1751997628785



  `nmcli device disconnect ifname eth0` ;;; Disconnect an interface = #cs/linux/networking 
ID: 1751997628789



- `$nmcli connection show` = Get a list of connections with their names, UUIDs, types and backing devices = #cs/linux/networking
  `nmcli connection up _name_or_uuid_` ;;; Activate a connection (i.e. connect to a network with an existing profile) = #cs/linux/networking 
  `nmcli connection delete _name_or_uuid_` ;;; Delete a connection = #cs/linux/networking 
  `nmcli device` ;;; See a list of network devices and their state = #cs/linux/networking 
  `nmcli radio wifi off` ;;; Turn off Wi-Fi = #cs/linux/networking 
