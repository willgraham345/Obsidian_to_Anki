...
class TestUtils:
    ...
    def test_write_safe_success(self, mock_remove, mock_rename, mock_open_file):
        write_mock = mock_open(read_data="test content").return_value
        read_mock = mock_open(read_data="test content").return_value
        mock_open_file.side_effect = [write_mock, read_mock]
        ...
        write_mock.write.assert_called_once_with("test content")
    ...
    def test_contained_in(self):
        ...
        assert contained_in((10, 11), spans_list) is True  # Within leeway (+1)
        assert contained_in((9, 11), spans_list) is True   # Within leeway
        assert contained_in((19, 21), spans_list) is True  # Within leeway
        assert contained_in((12, 15), spans_list) is False  # Outside leeway

    def test_findignore(self):
        pattern = re.compile(r"word")
        string = "This word is ignored. Another word here."
        # first 'word' is at (5,9) — put it in ignore_spans
        ignore_spans = [(5, 9)]
        results = [m.group(0) for m in findignore(pattern, string, ignore_spans)]
    ...
    def test_load_anki_success(self, mock_wait_for_port, mock_popen):
        ...
        result = load_anki(mock_config, 8765)
